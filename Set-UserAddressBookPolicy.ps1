            ...
            If($user.MailNickName){
                ...
                if ($userMailbox) {
                    ...
                    if ($counter -lt $DifferentialScope) {
                        ...
                        Set-Mailbox $user.UserPrincipalName -AddressBookPolicy $objAddressBookPolicy.Name
                        if($?){Write-Log -Message "$sString0;ABP:$($objAddressBookPolicy.Name);UserObjectId:$($user.ObjectId)" -LogLevel SUCCESS -ConsoleOutput}
                        $counter++
                    }
